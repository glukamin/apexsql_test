...
GO
create function dbo.ICE_SelectRenewalMaintDetails (@Order_ID int, @Today datetime)
returns table
as
return Select T.Order_ID, T.FirstName, T.LastName, T.Email, 
           ...
           From
           (Select Order_ID = O.Order_ID, FirstName = O.First_Name_Ship, LastName = O.Last_Name_Ship, Email = O.Email_Ship,
               ...
               ) T Group by T.Order_ID, T.FirstName, T.LastName, T.Email
GO
GRANT SELECT
	ON [dbo].[ICE_SelectRenewalMaintDetails_Test]
	TO [adrijan.bandu]
GO
GRANT SELECT
	ON [dbo].[ICE_SelectRenewalMaintDetails_Test]
