...
CREATE FUNCTION [dbo].[ICE_GetLicensePriceEnterprice]
(
	@Product_ID int, 
	@License_Type_ID int, 
	@OrderDetailsID int,
...
BEGIN
	...
	IF (@License_Type_ID NOT IN (0,1))
		BEGIN
			SET @BasePrice = ISNULL((SELECT Total FROM dbo.ICE_Order_Details iod WHERE iod.License_Type_ID = @License_Type_ID AND iod.Order_Detail_Id = @OrderDetailsID),0)
			SET @ResultPrice = @BasePrice
		END
	ELSE
		BEGIN
