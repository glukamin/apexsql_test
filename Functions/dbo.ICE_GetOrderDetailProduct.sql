...
CREATE FUNCTION [dbo].[ICE_GetOrderDetailProduct](@Order_Detail_ID int)
returns varchar(200)
as
Begin
declare @Result varchar(200), @Product_ID int, @License_Type_ID int, @Maintenance_ID int, @Shareit_ID int
select @Product_ID=Product_ID, @License_Type_ID=License_Type_ID, @Maintenance_ID=Maintenance_ID from ICE_Order_Details where Order_Detail_ID=ISNULL(@Order_Detail_ID, 0)
...
GO
GRANT EXECUTE
	ON [dbo].[ICE_GetOrderDetailProduct]
	TO [adrijan.bandu]
GO
GRANT EXECUTE
	ON [dbo].[ICE_GetOrderDetailProduct]
