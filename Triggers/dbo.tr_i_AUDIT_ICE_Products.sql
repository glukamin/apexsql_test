...
GO
CREATE TRIGGER [dbo].[tr_i_AUDIT_ICE_Products]
ON [dbo].[ICE_Products]
FOR INSERT
NOT FOR REPLICATION
As
...
-- Description:  INSERT TRIGGER for Table:  [dbo].[ICE_Products]
...
BEGIN
	...
	SELECT
		@AUDIT_LOG_TRANSACTION_ID,
		convert(nvarchar(1500), IsNull('[Product_ID]='+CONVERT(nvarchar(4000), NEW.[Product_ID], 0), '[Product_ID] Is Null')),
		'Product_ID',
		CONVERT(nvarchar(4000), NEW.[Product_ID], 0),
		'A'
		, CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[Product_ID], 0))
	FROM inserted NEW
	...
	SELECT
		...
		, CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[Product_ID], 0))
	FROM inserted NEW
	WHERE NEW.[Product_Alias] Is Not Null
	...
	SELECT
		...
		, CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[Product_ID], 0))
	FROM inserted NEW
	WHERE NEW.[Is_Private] Is Not Null
	...
	SELECT
		...
		, CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[Product_ID], 0))
	FROM inserted NEW
	WHERE NEW.[Default_Major_Build_Version] Is Not Null
...
End
GO
EXEC sp_settriggerorder @triggername= '[dbo].[tr_i_AUDIT_ICE_Products]', @order='Last', @stmttype='INSERT'
GO
