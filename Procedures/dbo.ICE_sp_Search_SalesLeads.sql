...
GO
CREATE PROCEDURE [dbo].[ICE_sp_Search_SalesLeads]
(
      @Sales_Lead_ID int = 0, 
...
BEGIN 
      ...
      SET @sql = @sql + ' 
            SELECT DISTINCT  TOP 1000
                  ...
                  [Date_Post_Download_Started],
                  [Lead_Source],
                  [CRM_ID],
                  [Do_Not_Contact],
                  [Last_Thank_You_Email],
                  [Lead_Source],
                  (
                        SELECT TOP 1 Download_Date 
                        FROM ICE_Downloads 
                        WHERE ICE_Downloads.Sales_Lead_Id = SalesLeadId 
                        ORDER BY Download_Date 
                  ) AS First_Download,                
                  ISNULL(S.Post_Download_Status_ID, 1) as Post_Download_Status_ID,
            ...
            INNER JOIN dbo.ICE_Downloads D on S.SalesLeadId = D.Sales_Lead_Id
              ...
              WHERE (1=1)'
      --Add parameters
      SET @sql = @sql + ' AND (SELECT TOP 1 Download_Date FROM ICE_Downloads D WHERE D.Sales_Lead_ID = SalesLeadId ORDER BY Download_Date) BETWEEN @Last_Download_From and @Last_Download_To '
      IF @Sales_Lead_ID > 0
...
GO
GRANT EXECUTE
	ON [dbo].[ICE_sp_Search_SalesLeads]
	TO [adrijan.bandu]
GO
GRANT EXECUTE
	ON [dbo].[ICE_sp_Search_SalesLeads]
