...
GO
CREATE procedure [dbo].[ICE_sp_SalesByProductExcludingBundle]
(
	@DateBegin datetime, 
	@DateEnd datetime
)
...
(
	...
	Select 
		...
		Total = SUM
				 (
					ROUND(ICE_Extension_Details.Total * 
					(1 - (ISNULL(ICE_Orders.Discount, 0)/100)), 0)
				 )
	from 
		ICE_Orders inner join 
...
where 
	dbo.ICE_IsProductBundle(T.Product_ID) = 0
...
GO
GRANT EXECUTE
	ON [dbo].[ICE_sp_SalesByProductExcludingBundle]
	TO [adrijan.bandu]
GO
GRANT EXECUTE
	ON [dbo].[ICE_sp_SalesByProductExcludingBundle]
