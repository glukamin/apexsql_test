...
GO
Create procedure [dbo].[ICE_sp_MonthlyAvgQtyPerOrder](@DateBegin datetime, @DateEnd datetime)
as
...
group by ICE_Order_Details.Order_ID, Year(ICE_Orders.Order_Date) * 100 + Month(ICE_Orders.Order_Date)
) T group by T.Month
GO
GRANT EXECUTE
	ON [dbo].[ICE_sp_MonthlyAvgQtyPerOrder]
	TO [adrijan.bandu]
GO
GRANT EXECUTE
	ON [dbo].[ICE_sp_MonthlyAvgQtyPerOrder]
